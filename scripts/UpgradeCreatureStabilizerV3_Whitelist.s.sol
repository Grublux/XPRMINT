...
/// @notice Upgrade script for STAB_V3 to the implementation that includes whitelist-based tester gating.
/// @dev This script:
...
///      - Calls upgradeAndCall(proxy, newImpl, "") (no re-init)
///      - Verifies the implementation changed
///      - Performs a light sanity check by calling isWhitelistEnabled() on the proxy
contract UpgradeCreatureStabilizerV3_Whitelist is Script {
    ...
    function run() external {
        ...
        console2.log("[STAB_V3 WHITELIST UPGRADE] Proxy:", stabProxy);
        console2.log("[STAB_V3 WHITELIST UPGRADE] Deployer:", deployer);
        ...
        console2.log("[STAB_V3 WHITELIST UPGRADE] ProxyAdmin:", proxyAdminAddr);
        ...
        console2.log("[STAB_V3 WHITELIST UPGRADE] ProxyAdmin.owner():", adminOwner);
        ...
        console2.log("[STAB_V3 WHITELIST UPGRADE] Current implementation:", oldImpl);
        ...
        console2.log("[STAB_V3 WHITELIST UPGRADE] New implementation:", newImplAddr);

        if (newImplAddr == oldImpl) {
            console2.log("[STAB_V3 WHITELIST UPGRADE] New implementation matches current implementation. No upgrade performed.");
            vm.stopBroadcast();
            return;
        }
        ...
        console2.log("[STAB_V3 WHITELIST UPGRADE] Implementation after upgrade:", implAfter);
        require(implAfter == newImplAddr, "CreatureStabilizerV3: implementation did not update");

        // --- 7. Post-upgrade sanity check: try calling isWhitelistEnabled() via proxy ---
        CreatureStabilizer stab = CreatureStabilizer(stabProxy);

        try stab.isWhitelistEnabled() returns (bool enabled) {
            console2.log("[STAB_V3 WHITELIST UPGRADE] isWhitelistEnabled():", enabled);
        } catch {
            console2.log("[STAB_V3 WHITELIST UPGRADE] isWhitelistEnabled() reverted (unexpected)");
        }

        console2.log("[STAB_V3 WHITELIST UPGRADE] Upgrade completed successfully.");
    }
}

...
/**
 * @title FinalUpgradeSolution
 * @notice Upgrades contracts using already-deployed implementations
 * @dev This script assumes ownership has been transferred manually.
 *      If ownership transfer fails, it will show clear error messages.
 */
contract FinalUpgradeSolution is Script {
    function run() external {
        ...
        if (newCatalogImpl == address(0) || newItemTokenImpl == address(0)) {
            ...
            vm.stopBroadcast();
            console.log("New ItemCatalog impl:", newCatalogImpl);
            console.log("New ItemToken1155 impl:", newItemTokenImpl);
        } else {
        ...
        if (catalogAdminOwner != deployer || itemTokenAdminOwner != deployer) {
            console.log("\nERROR: Ownership not transferred!");
            console.log("You need to transfer ownership of ProxyAdmins to deployer first.");
            console.log("See scripts/OWNERSHIP_TRANSFER_GUIDE.md for instructions.");
            vm.stopBroadcast();
            return;
        }

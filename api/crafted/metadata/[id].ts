...
const NPC_CUSTOM_NAMES: Record<number, string> = {
  // TODO (later): populate from on-chain MasterCrafterV1 or DB
};
...
const NPC_CUSTOM_NAMES: Record<number, string> = {
  // TODO (later): populate from on-chain MasterCrafterV1 or from a DB/indexer
};
...
function buildMetadataForToken(tokenId: number): CraftedMetadata {
  // TODO (later): derive from on-chain MasterCrafterV1 + NPC-721:
  // - recipeId
  // - npcTokenId (seat owner)
  // - inputAmountLocked (NGT Locked)
  // - unlockAt

  const recipeId = 1; // stub for now
  const npcTokenId = 1; // stub for now

  const recipe = RECIPE_DEFAULTS[String(recipeId)];
  const npcName = getNpcDisplayName(npcTokenId);

  const unlockAtUnix = 0; // TODO: positions(tokenId).unlockAt
  const unlocksInSeconds = 0; // TODO: compute unlockAt - now

  return {
    name: npcName, // NFT display name defaults to NPC display name
    description: recipe.description,
    image: recipe.image,
    external_url: `https://www.xprmint.com/crafted/${tokenId}`,
    attributes: [

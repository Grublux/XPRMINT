...
interface CraftedMetadata {
  name: string;
  description: string;
  image: string;
  external_url: string;
  attributes: Attribute[];
  properties_for_your_ui_only?: Record<string, string | number | null>;
}

// Static defaults for Recipe #1 (can be wired to on-chain later)
...
// Placeholder custom names (later: load from DB/indexer or on-chain)
...
function buildMetadataForToken(tokenId: number): CraftedMetadata {
  ...
  const unlockAtUnix = 0; // TODO when wiring on-chain
  const unlocksInSeconds = 0;

  return {
    // Name: NPC display name (user-customizable later)
    name: npcName,
    description: recipe.description,
    image: recipe.image,
    // NOTE: external_url uses crafted.xprmint.com but can be changed later via env
    external_url: `https://crafted.xprmint.com/crafted/${tokenId}`,
    attributes: [

    ...
    try {
      ...
      const tokenPromises = verifiedTokenIds.map(async (id: bigint) => {
          ...
          try {
            ...
            // Convert relative URLs to absolute (if metadata is from our API, make image URL absolute)
            if (imageUrl && !imageUrl.startsWith('http://') && !imageUrl.startsWith('https://') && !imageUrl.startsWith('data:')) {
              // If it's a relative path, try to make it absolute based on the metadata source
              if (metadataUri.startsWith('http')) {
                try {
                  const baseUrl = new URL(metadataUri).origin;
                  imageUrl = new URL(imageUrl, baseUrl).href;
                } catch (e) {
                  // If URL construction fails, try appending to the metadata base
                  if (metadataUri.includes('/api/crafted/')) {
                    // For our API routes, images might be relative to the API base
                    imageUrl = `https://xprmint-metadata-oych.vercel.app${imageUrl.startsWith('/') ? '' : '/'}${imageUrl}`;
                  }
                }
              } else if (metadataUri.startsWith('/api/')) {
                // For local API routes, images are likely on the metadata server
                imageUrl = `https://xprmint-metadata-oych.vercel.app${imageUrl.startsWith('/') ? '' : '/'}${imageUrl}`;
              }
            }
            ...
            console.log('[useCoinTokens] Coin', id.toString(), 'imageUrl:', imageUrl, 'from metadata:', metadata);
